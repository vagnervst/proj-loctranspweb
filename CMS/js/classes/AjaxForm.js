function AjaxForm() {
    ...
    this.get_registro_selecionado_json = function(botao_edicao) {
        var registro_selecionado = $(botao_edicao).parents(".registro")[0];        
        
        var lista_registros_tabela = $(this.tabela).find(".registro");
        
        var indice_registro_selecionado = lista_registros_tabela.index(registro_selecionado);

        var json_registro_selecionado = this.jsonRegistros[ indice_registro_selecionado ];                
        
        return json_registro_selecionado;
    };
    
    this.actionBotoesEdicao = function(botao) {
        $(this.formulario).find('input:checkbox').removeAttr('checked');                        
        
        var json_registro_selecionado = this.get_registro_selecionado_json(botao);        
        
        $( this.formulario ).data( "id", json_registro_selecionado.id );

        var lista_dados_para_form = [];

        for( var i = 0; i < this.relacao_campo_propriedade.length; ++i ) {
            var campo_propriedade = this.relacao_campo_propriedade[i];

            lista_dados_para_form[ campo_propriedade.nomeCampo ] = json_registro_selecionado[ campo_propriedade.propriedade ];
        }

        this.lista_chave_valor_formulario = lista_dados_para_form;
        this.set_campos_formulario();
        this.alterar_modo_crud_para( "update" );
    };
    ...
    this.prepararBotoesEdicao = function() {
        ...
        $(this.tabela).on("click", ".botao-editar", function() {
            ...
            var botaoEdicao = this;
            if( self.preEdicaoFormulario !== undefined ) {                                
                
                var dados = {
                    registroSelecionado: self.get_registro_selecionado_json(botaoEdicao)
                };
                
                self.preEdicaoFormulario( dados, function() {
                    self.actionBotoesEdicao(botaoEdicao);
                });
                
            } else {
                self.actionBotoesEdicao(this);
            }
